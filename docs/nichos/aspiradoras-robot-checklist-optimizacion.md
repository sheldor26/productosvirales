# Checklist de optimización SEO — Aspiradoras Robot (18 fichas)

> Tracking de la optimización 1 a 1 de todas las aspiradoras robot del catálogo.
> Método por ficha: datos reales de ML (ficha + opiniones) → cruzar keywords de
> `Keywords/Aspiradoras Robot/` (CSV) **con** Ubersuggest (locId 2032, es) → checklist
> completo (seoTitle/meta/H1/OG, schema Product+FAQPage, 3-5 links internos, specs, faq,
> customerReviews reales, humanizer) → verificar (lint + parseo + links) → commit.
> Detalle del método y estrategia: `docs/nichos/aspiradoras-robot-seo-ubersuggest.md`.

## Estado: 15 / 18 completas — ✅ Tier 1 Xiaomi + Tier 2 Atma + Tier 3 Gadnic + Tier 4 Samsung COMPLETOS

### ✅ Ya optimizadas (15)

| # | Producto | ID | Marca |
| --- | --- | --- | --- |
| ✅ | Kärcher RCV 1 | MLA42103831 | Kärcher |
| ✅ | Samsung Jet Bot+ LiDAR | MLA26504431 | Samsung |
| ✅ | Midow MDW/RZT | MLA48378491 | Midow |
| ✅ | Gadnic AspiRob 3 Modos | MLA42045783 | Gadnic |
| ✅ | Xiaomi S40 Pro 15000 Pa | MLA66281403 | Xiaomi |
| ✅ | Xiaomi S40c E101 | MLA61420449 | Xiaomi |
| ✅ | Xiaomi X20 Pro D102GL | MLA44240016 | Xiaomi |
| ✅ | Xiaomi X20 Max | MLA44235159 | Xiaomi |
| ✅ | Xiaomi S20 negra | MLA44714806 | Xiaomi |
| ✅ | Atma Atar21c1pi | MLA20033508 | Atma |
| ✅ | Atma Atar21c1dh | MLA18642318 | Atma |
| ✅ | Gadnic 5 Modos | MLA45951645 | Gadnic |
| ✅ | Gadnic 5000 Pa LIDAR 360 | MLA62126950 | Gadnic |
| ✅ | Gadnic AspiRob Mapeo App | MLA36838658 | Gadnic |
| ✅ | Samsung Powerbot E 2 en 1 | MLA18917302 | Samsung |

### ⬜ Pendientes (3) — en orden de prioridad (ROI por Ubersuggest)

**Tier 1 — Xiaomi** (`aspiradora robot xiaomi` 2.900/mes, dificultad 13 — el mejor ROI del nicho)

| # | Producto | ID | Estado |
| --- | --- | --- | --- |
| 1 | Xiaomi Robot Vacuum S40 Pro 15000 Pa | MLA66281403 | ✅ |
| 2 | Xiaomi Robot Vacuum S40c E101 | MLA61420449 | ✅ |
| 3 | Xiaomi Robot Vacuum X20 Pro D102GL | MLA44240016 | ✅ |
| 4 | Xiaomi Vacuum X20 Max | MLA44235159 | ✅ |
| 5 | Xiaomi Vacuum S20 negra | MLA44714806 | ✅ |

**Tier 2 — Atma** (`aspiradora robot atma` 1.000/mes, dificultad 13)

| # | Producto | ID | Estado |
| --- | --- | --- | --- |
| 6 | Atma Atar21c1pi Trapeadora Anti-choque | MLA20033508 | ✅ |
| 7 | Atma Atar21c1dh | MLA18642318 | ✅ |

**Tier 3 — Gadnic** (`aspiradora robot gadnic` 1.900/mes — ya rankeable; quedan 3)

| # | Producto | ID | Estado |
| --- | --- | --- | --- |
| 8 | Gadnic 5 Modos Trapeo App | MLA45951645 | ✅ |
| 9 | Gadnic 5000 Pa LIDAR 360 Autolimpiante | MLA62126950 | ✅ |
| 10 | Gadnic AspiRob Recargable Mapeo App | MLA36838658 | ✅ |

**Tier 4 — Samsung** (`aspiradora robot samsung` 1.300/mes; queda 1)

| # | Producto | ID | Estado |
| --- | --- | --- | --- |
| 11 | Samsung Powerbot E 2 en 1 | MLA18917302 | ✅ |

**Tier 5 — Fika + genérica** (long-tail `robot aspiradora y trapeadora`, mascotas)

| # | Producto | ID | Estado |
| --- | --- | --- | --- |
| 12 | Fika Nexos Giroscópica | MLA63784269 | ⬜ ← SIGUIENTE |
| 13 | Fika SENSE Antichoque | MLA54522658 | ⬜ |
| 14 | Robot Trapeadora Láser/Mapeo (genérica) | MLA53579189 | ⬜ |

---

### Cómo avanzamos
Cada vez: Juan pasa la ficha + opiniones de ML del producto del día → Claude optimiza siguiendo
el checklist → marca ✅ acá → siguiente. Se actualiza este archivo al cerrar cada una.
