...
- [x] **3.2 `ProductDetail` → RSC + islas cliente** ✅ 2026-09-21 — `ProductDetail.tsx` (913 líneas) sacó el `"use client"`: contenido, tablas, pros/cons, comparar, para-quién-es, artículo, reviews, specs, FAQ y banda CTA ahora son Server Component (JSON-LD ya estaba en server desde `page.tsx`, no era necesario moverlo). Solo 3 cosas forzaban el cliente y quedaron aisladas: el botón de guardar (`SaveHeartButton.tsx`, nuevo, chico), el registro de "visto recientemente" (`RecentlyViewedRecorder.tsx`, nuevo, invisible) y GSAP — reemplazado por CSS puro (`.detail-reveal` + `animationDelay` inline por sección, `src/app/globals.css`, mismo patrón `.hero-rise` ya usado en `HeroBanner.tsx`, comentario propio "replaces GSAP timeline"). `ProductGallery`, `ShareButtons`, `PriceHistoryChart`, `CouponBadge`, `RecentlyViewed`, `StickyMobileCta` ya eran Client Components propios — no hacía falta tocarlos, solo dejaban de estar "atrapados" bajo un padre cliente. `AffiliateLink` no tenía `"use client"` ni hooks — no hacía falta nada. Efecto colateral: `gsap`/`@gsap/react` quedaron sin ningún import en todo `src/` (eran el único uso) — no se sacó de `package.json` todavía, queda pendiente confirmarlo con Juan antes de tocar dependencias.
  - Archivos: `src/components/products/ProductDetail.tsx`, `src/components/products/SaveHeartButton.tsx` (nuevo), `src/components/products/RecentlyViewedRecorder.tsx` (nuevo), `src/app/globals.css`.
  - Verificado en vivo: ficha con todas las secciones opcionales pobladas (pros/cons, comparar, reviews, specs, FAQ) y otra con solo algunas — layout idéntico al de antes, reveal CSS corre y termina en `opacity:1`, botón de guardar y "visto recientemente" persisten en localStorage, `<details>` de FAQ sigue abriendo/cerrando, links de afiliado con `rel="sponsored nofollow noopener"` intacto, sin errores nuevos en consola. `npx tsc --noEmit` y `npm run build` en verde (829+ fichas siguen generándose estático).
  - Riesgo real: bajo — refactor mecánico de un solo archivo (mover JSX, no reescribirlo), sin cambiar la firma de `ProductDetail` ni tocar `page.tsx` ni ningún hijo ya-cliente.
...
### Iteración 30 (2026-08-01) — primera ronda con investigación externa real

**Cambio de proceso, corrección de Juan:** "no veo que lances agentes para profundizar / buscar ideas en la web" — hasta acá las tres fuentes (Codex, Gemini, Claude) venían siendo variaciones del mismo insumo (leer el repo local). Desde esta ronda, la contribución propia de Claude incluye un Agent con WebSearch investigando patrones de UX/CRO reales en sitios de afiliados/comparadores comparables (Wirecutter, RTINGS, CamelCamelCamel), qué evitar (dark patterns, FTC), y qué es evidencia real vs. hype. Regla guardada en memoria: `loop-mejora-continua-claude-tambien-investiga-afuera`.

**Tres propuestas, dos implementadas, dos refutadas de la investigación externa:**
- **Codex:** módulo "Compará antes de salir" en ficha con alternativas automáticas — verificado que hoy la ficha solo compara si hay `relatedProducts` explícitos, nunca automáticos. Esfuerzo bajo-medio, riesgo medio (podría distraer del CTA). **Queda en la cola**, no se implementó esta ronda.
- **Gemini:** replicar el badge "Mínimo histórico" en la ficha — existe en las cards de grilla pero no en la zona de decisión de compra de la ficha, pese a que el dato (`priceHistory.verdict.tone`) ya llega ahí. **Implementado.**
- **Investigación externa (Claude):** 5 ideas con fuente citada. Dos refutadas contra el código antes de tocar nada: "botón de compartir por WhatsApp" YA EXISTE en `ShareButtons.tsx` (primer ícono); "verificado hace X horas" no es honesto con la granularidad real del sitio (Bright Data corre 3x/semana, solo hay fecha, no hora — mostrarlo habría sido falsa precisión). Una implementada (precio promedio). Una guardada como regla preventiva (nunca urgencia sin dato real — `loop-mejora-continua-nunca-urgencia-falsa`). Una queda en la cola (sticky mini-bar en tablas comparativas largas, evidencia de Baymard, esfuerzo medio).

**Implementado:**
1. Badge "Mínimo histórico" (mismo `Badge variant="price-low"` que `ProductCard.tsx`) en `ProductDetail.tsx`, junto al precio, cuando `priceHistory?.verdict.tone === "good"`.
2. `PriceChartData` (`price-history.ts`) suma `avg` (promedio simple de los precios registrados) y `rangeDays` (días reales entre el primer y último punto — nunca un período fijo tipo "90 días" inventado). En la ficha, línea "Promedio de los últimos N días: $X" cuando `rangeDays >= 7`. **Siempre visible cuando aplica, no solo cuando favorece el precio actual** — mismo principio de honestidad que ya rige el gráfico de precio completo (memoria `price-chart-honestidad-sobre-conversion`): no ocultar datos por conversión.

**Verificado en navegador:** ficha del Lattafa Khamrah Qahwa (19 puntos de historial) muestra "Promedio de los últimos 103 días: $ 50.112" sin badge (verdict neutral, correcto). Ficha del Monitor Noblex NXSM2700 (verdict "good") muestra AMBOS: badge verde "Mínimo histórico" + "Promedio de los últimos 22 días: $ 227.181", sin pisarse con el cupón ni el resto del layout — confirmado con screenshot. Gotcha de herramienta nuevo: `get_page_text` extrajo mal el contenido de `<main>` en dos fichas (devolvió solo header/footer) mientras el DOM real (`elementsFromPoint`, `body.innerHTML.length`, `h1.textContent`, screenshot) confirmaba contenido completo y correcto — no relacionado con HMR (persistió en pestañas frescas), causa exacta no determinada (posible interacción con animaciones GSAP de entrada), la solución fue verificar por DOM/screenshot en vez de confiar en esa herramienta puntual. `tsc --noEmit`, `eslint` y `npm run build`, todos limpios.

**Con 44 features implementadas, 43 commits locales.**

### Iteración 31 (2026-08-01) — Codex retoma la cola, Gemini falla, investigación web refuta dos ideas

**Gemini falló** con el error de permisos headless de siempre (descartado sin relanzar). **Codex** retomó la cola de la ronda 30 (sticky mini-bar en tablas comparativas largas, evidencia de Baymard) pero la aplicó a guías en vez de al comparador de categoría, reusando `StickyBuyBar.tsx` ya existente en lugar de construir algo nuevo — verificado que hoy esa barra solo aparece por `scrollY > 600`, sin relación con dónde está el lector.

**Investigación externa (Claude, ángulo nuevo: micro-interacciones mobile + retención sin cuentas):** 5 ideas con fuentes citadas (LogRocket, Baymard, NNGroup, Smashing Magazine). Dos refutadas contra el código antes de tocar nada: "visto recientemente" YA EXISTE (`RecentlyViewed.tsx`, implementado en sesiones previas — la investigación no tenía forma de saberlo sin leer el repo); "skeleton screens para el precio en vivo y el gráfico de historial" no aplica porque ninguno de los dos hace fetch async en el cliente (`{{precio:MLA}}` se resuelve server-side vía `injectLivePrices`, `priceHistory` llega como prop ya computada) — no hay spinner que reemplazar. Una idea (no construir "Quick View", evidencia de Baymard de que confunde en catálogos spec-driven) es una regla preventiva sin código, no una feature. Una idea (Vibration API en CTAs) quedó descartada por evidencia débil (un solo case study, sin soporte en iOS Safari, ROI incierto).

**Implementado:** `GuideRenderer.tsx` marca las tablas comparativas con la clase `guide-comparison-table`. `StickyBuyBar.tsx` ahora usa un `IntersectionObserver` sobre esas tablas: la barra aparece TAMBIÉN mientras una tabla está en pantalla (aunque sea antes de los 600px de scroll), no solo por posición de scroll — cambio aditivo, nunca oculta la barra en un caso donde ya aparecía. `data-cta-location` distingue `"sticky-table"` de `"sticky"` para medir el efecto por separado con el tracking existente.

**Gotcha de verificación nuevo, importante para rondas futuras:** `window.scrollTo()` disparado vía `javascript_exec` NO dispara el callback de `IntersectionObserver` de forma confiable en el entorno de browser automatizado usado para verificar — ni un observer manual de prueba se disparaba tras esperar y reintentar varias veces. Un scroll real simulado con el gesto de `computer scroll` sí lo dispara correctamente y de inmediato. Cualquier feature basada en `IntersectionObserver` debe verificarse con scroll real, no con `scrollTo()` por JS — de lo contrario el hallazgo "no funciona" es falso.

**Verificado en navegador (mobile 375px, guía real `mejores-freidoras-de-aire-argentina`):** con scroll real hasta la tabla comparativa (14810-16322px), la barra muestra `data-cta-location="sticky-table"`; scrolleando más allá de la tabla (pero todavía pasado los 600px), vuelve a `"sticky"`. `tsc --noEmit`, `eslint` y `npm run build`, todos limpios. Se evaluó agregar `env(safe-area-inset-bottom)` a la barra (como ya tiene `StickyMobileCta.tsx` en fichas) pero se descartó por estar fuera del alcance verificado de esta ronda — es un gap preexistente, no algo que este cambio empeore.

**Con 45 features implementadas, 44 commits locales.**

### Iteración 32 (2026-08-01) — Codex y Gemini convergen en la cola, investigación de confianza detecta un posible dato no verificable

**Codex y Gemini propusieron independientemente la misma idea**, retomando la cola de la ronda 30 ("Compará antes de salir"): dar ordenar/comparar en la sección automática de la ficha, ubicada al final absoluto de la página (después del CTA principal, la tabla curada, el historial de precio y las guías relacionadas) — la mitigación de riesgo de "no distraer del CTA" que frenó la idea en la ronda 30 ya estaba resuelta por la posición existente. Ambos verificaron correctamente: `related` (4 productos automáticos de la misma categoría) se renderizaba como `ProductGrid` pasivo en `page.tsx:347-353`.

**Investigación externa (Claude, ángulo confianza/E-E-A-T):** 6 ideas con fuentes citadas. Dos ya sustancialmente resueltas: el disclaimer de afiliados en guías ya incluye declaración de independencia editorial explícita (no es boilerplate legal puro); página de metodología ya existe (`/sobre-nosotros`, con sección "Cómo seleccionamos los productos" y "Cómo ganamos dinero"). Una detección relevante sin código: la frase "Probamos muchos nosotros mismos" en `/sobre-nosotros` afirma testeo físico que no tiene respaldo en ningún otro lugar del proyecto (el proceso documentado es cruce de datos: ML + web del fabricante + Bright Data, sin evidencia de unboxing) — **no se tocó**, porque es una afirmación sobre la práctica personal de Juan que no se puede confirmar ni refutar desde el código. Flageada como tarea aparte (`task_aedfc5d0`) para que él la confirme o corrija. Una idea (bio del curador con foto real) requiere una decisión personal de Juan (publicar su foto), no algo para implementar sin consultar. Una idea (evitar trust badges genéricos) es una regla preventiva sin código — el sitio no tiene ninguno hoy, nada que quitar.

**Implementado:** en `src/app/producto/[slug]/page.tsx`, la sección "Productos similares" pasó de `ProductGrid` a `SortableProductGrid` — cambio de una línea, mismo componente construido en la ronda 26, sin código nuevo.
...
### Iteración 36 (2026-08-01) — primera ronda con cola en cero: tres ideas distintas, una implementada, dos evaluadas y descartadas de esta ronda

Con la cola vacía por primera vez, las tres fuentes propusieron ideas genuinamente distintas entre sí (a diferencia de rondas anteriores donde Codex/Gemini convergían en la cola compartida):

- **Codex:** chips de filtro rápido por señales reales de compra (envío gratis, mínimo histórico, con descuento, 4.8+, más vendidos) en `SortableProductGrid.tsx` — verificó correctamente que las 6 opciones de orden existentes (`product-sort.ts`) no son filtros, y que las señales ya viven en el DTO de tarjeta sin usarse para filtrar. **Implementado.**
- **Investigación externa (ángulo nuevo: cross-sell/bundles en sitios de afiliados sin carrito propio, agente con WebSearch):** evidencia de Baymard/Carnegie Mellon descarta un sistema de "comprados juntos" data-driven (requiere datos de checkout que este sitio no tiene; forzarlo con datos flojos sería inventar prueba social falsa). Sí encontró evidencia razonable para un bloque editorial "Kit recomendado" (2-3 productos complementarios curados a mano, no por algoritmo) en guías pillar con caso de uso de combo real. **No implementado esta ronda**: elegir qué productos van juntos es una decisión de contenido/curación editorial, no una feature de UX/código — cae fuera del alcance de este loop por la regla `loop-mejora-continua-no-tocar-contenido-guias` (agregar un bloque nuevo con picks curados es una decisión editorial tanto como reescribir prosa existente). Queda anotado para que Juan lo derive a `/optimizador-guias-pv` si le interesa, no para que el loop lo tome.
- **Gemini/agy (recuperado esta ronda tras varias fallas):** búsqueda predictiva (autocomplete en vivo) en el header — verificó correctamente que `SearchInput.tsx:24` solo reacciona a `onSubmit` y que `Header.tsx:31-34` hace un `router.push` de página completa, sin ningún estado de sugerencias en vivo. Propuesta sólida y bien evidenciada, pero de esfuerzo mayor (nuevo Route Handler público sobre el catálogo, popover con debounce, manejo de foco/teclado, variante mobile expandible) — no es una extensión de una hora de un componente existente como las últimas features. **Queda en la cola para una ronda dedicada**, mismo criterio que se usó con el filtro de precio en la ronda 33→34.

**Implementado:** `src/lib/product-signals.ts` (nuevo) con predicados puros sobre `CardProduct` y `buildAvailableSignals()` (mismo criterio que `price-buckets.ts`: un chip solo aparece si al menos un producto del listado lo cumple). Chips multi-selección (lógica Y) integrados en `SortableProductGrid.tsx`, por lo que aparecen gratis en categorías, búsqueda, guardados y "productos similares" de ficha. Umbrales de rating (4.8+) y ventas (1000+) elegidos verificando la distribución real del catálogo (no a ojo): 164/299 productos con rating cargado llegan a 4.8+; de los 300 productos, solo 116 tienen `soldQuantity` cargado y de esos 77 llegan a 1000+ — los que no tienen el dato quedan simplemente afuera del filtro, no se les asigna un valor inventado.

**Verificado en navegador (categoría gaming, 53 productos):** "Envío gratis" sola → 50 (coincide con la ronda 29); sumando "4.8 o más" → 34; sumando "Con descuento" → 14, nunca se vacía; sacando los tres chips → vuelve a 53. Convive sin conflicto con el filtro de precio y el orden. Gotcha de herramienta nuevo, mismo patrón que el #9: el screenshot del navegador automatizado se quedó en negro sólido pese a contenido real confirmado por `innerHTML.length` y queries DOM directas — se verificó todo el flujo por DOM (clicks vía `.click()` + lectura de `aria-pressed` y conteo de links `/producto/`) en vez de confiar en la captura visual. `tsc --noEmit`, `eslint` y `npm run build`, todos limpios.

**Con 51 features implementadas, 51 commits locales.**

### Iteración 37 (2026-08-01) — Codex y Gemini convergen dos rondas seguidas, investigación de accesibilidad refuta su propia hipótesis

**Codex y Gemini propusieron, otra vez de forma independiente, la misma idea que ya había quedado en cola desde la ronda 36:** búsqueda predictiva (autocomplete) en el header. Ambos verificaron lo mismo: `SearchInput.tsx` solo reaccionaba a `onSubmit`, `Header.tsx` hacía `router.push` de página completa sin estado de sugerencias en vivo. Con doble convergencia y evidencia sólida de ambos, se implementó esta ronda.

**Investigación externa (ángulo nuevo: accesibilidad como palanca de conversión, agente con WebSearch):** trajo evidencia real y bien fundamentada (WebAIM Million 2026: contraste, alt text, labels y botones-ícono vacíos como los 6 errores que explican el 96% de los fallos de a11y en la web; NN/g sobre botones solo-ícono; FTC multando a accessiBe con USD 1M por el overlay de accesibilidad, confirmando que nunca instalar ese tipo de parche). Recomendó agregar `aria-label` a los botones solo-ícono de guardar/comparar en las product cards. **Verificado contra el código y refutado**: `ProductCard.tsx` ya tiene `aria-label` en ambos botones (líneas 161 y 189, de una ronda anterior), y todos los demás botones-ícono del sitio (`Header.tsx`, chips de `SortableProductGrid.tsx`, "Cargar más" y "Comparar" de `HomeFeed.tsx`, "Reintentar" de `SavedProductsView.tsx`) tienen texto visible o `aria-label` ya puesto. La investigación acertó en la evidencia general (es un problema real y común en e-commerce) pero erró en la aplicación puntual a este sitio, que ya lo tenía resuelto de sesiones previas. No se implementó nada de esta rama — queda documentado el chequeo para no repetirlo a ciegas en una ronda futura.

**Implementado:** `src/app/api/search/suggest/route.ts` (nuevo, público, sin secret — a diferencia de `/api/search` que consulta MercadoLibre en vivo) reusa el mismo matcheo exacto-con-fallback-a-fuzzy que ya usa `HomeFeed` sobre `?q=`. `SearchInput.tsx` gana debounce de 250ms, popover accesible (`role="combobox"`/`"listbox"`, `aria-controls` con id único vía `useId` porque el header monta la versión desktop y mobile del input a la vez), navegación por teclado (flechas, Enter, Escape) y click directo a la ficha. El submit tradicional a `/?q=...` queda intacto como fallback si el fetch falla o no se eligió sugerencia — nunca se rompe el camino que ya funcionaba.

**Verificado en navegador:** `/api/search/suggest?q=monitor` devuelve 6 resultados reales con precio; tipeando "monitor" aparece el popover con miniatura + categoría + precio (confirmado con `zoom` que las imágenes cargan, pese a un glitch visual inicial del screenshot); ArrowDown + submit navega a la ficha resaltada; click directo en una sugerencia también navega (probado con "aire" → Horno Atma con freidora); con menos de 2 caracteres o sin sugerencia elegida, el submit sigue yendo a `/?q=...`. Gotcha nuevo: el `computer key: "Down"` del navegador automatizado no disparó el `onKeyDown` de React (posible problema de evento no confiable/sintético), mientras que un `KeyboardEvent('keydown', {key: 'ArrowDown'})` disparado por JS sí funcionó — se verificó todo el flujo de teclado por dispatch de eventos en vez de la tecla simulada del tool. `tsc --noEmit`, `eslint` (incluido el warning de `jsx-a11y` sobre combobox, resuelto con `aria-controls`) y `npm run build`, todos limpios.

**Con 52 features implementadas, 52 commits locales. Cola en cero otra vez.**

### Iteración 38 (2026-08-01) — Codex encuentra un bug real, Gemini falla, investigación de personalización acierta con evidencia sólida

**Codex** propuso "Alternativas disponibles" en fichas sin stock, y de paso destapó un bug real: `related` (la sección automática "Productos similares") nunca filtraba `priceStatus`, así que una ficha sin stock podía recomendar otro producto tampoco disponible como "alternativa". **Gemini falló** de nuevo con el error de permisos headless (gotcha #6). **Investigación externa (ángulo nuevo: personalización desde navegación anónima sin cuentas):** trajo evidencia sólida de NN/g — la personalización oculta genera más desconfianza que conversión, la gente prefiere elegir ella misma, pero explicitar en qué se basa una sugerencia le da credibilidad. Con catálogo chico y sin cuentas, recomendó no inferir nada oculto (precio, intención) y solo mostrar en texto llano un patrón real y verificable: qué categoría se repite en "Visto recientemente".

**Implementado (dos features, cada una con su commit):**
1. `related` ahora excluye `priceStatus === "out_of_stock"` siempre (nunca recomendar un callejón sin salida). Cuando el producto actual está sin stock y quedan 2+ alternativas, la grilla se adelanta arriba (después del aviso "avisame", antes de las guías) con copy propio "Mientras tanto, mirá estas alternativas" — mitigación de Codex: con 0 o 1 alternativa no se promueve.
2. `RecentlyViewed.tsx` detecta si 2+ productos del historial comparten categoría y agrega un link transparente "Estuviste mirando X — ver todos los modelos" hacia `/categoria/X`, suprimido si esa categoría coincide con la de la ficha actual (evita redundancia).

**Gotcha de verificación nuevo, importante (#13):** el dev server llevaba corriendo sin reiniciar desde el viernes anterior. Para estos dos cambios, la navegación real en el navegador automatizado (incluso en pestañas 100% nuevas, nunca visitadas, con cache-busting en la URL) mostraba contenido stale (sin los cambios), mientras que `curl` y `fetch()` ejecutado DESDE la misma página, contra la misma URL, devolvían el HTML correcto y actualizado. Se resolvió levantando un build de producción real (`next start` en un puerto aparte) y verificando ahí — protocolo ya conocido (gotcha #3), pero esta es la primera vez que ni siquiera una pestaña nueva alcanzó para esquivarlo; cuando el dev server lleva muchas horas/días corriendo con cientos de ediciones encima, conviene sospechar directamente del dev server y saltar a un build real en vez de seguir peleando con pestañas nuevas.
...
### Iteración 40 (2026-08-01) — Codex extiende el fix de sin-stock un nivel más arriba, investigación de re-enganche confirma la dirección ya tomada, Gemini falla

**Codex** encontró la continuación exacta de las rondas 38-39: la ficha ya resolvió no empujar el click afiliado sin stock, pero las cards de listados (categoría, home, búsqueda) tenían el mismo agujero un nivel arriba — imagen y título de la card ya enlazan a la ficha interna, pero el botón redondo de la esquina seguía yendo directo a MercadoLibre sin mirar `priceStatus`. **Gemini falló** de nuevo (gotcha #6). **Investigación externa (ángulo re-enganche sin cuentas/push):** confirmó en gran parte que la dirección ya tomada por el sitio es la correcta según evidencia (Baymard: "Guardados" sin cuenta > "wishlist"; NN/g: nombrarlo así y no pedir login es lo que la evidencia recomienda, no solo una limitación técnica aceptable), y aportó dos ajustes nuevos: copy explícito sobre el mecanismo para evitar el efecto "vigilado", y el techo técnico real de Safari/iOS (localStorage se borra a los 7 días sin visitar) como limitación a tolerar, no a "arreglar".

**Implementado (dos features):**
1. El botón redondo de `ProductCard` (usado en toda card de listado) ahora es un `Link` interno a la ficha cuando `priceStatus === "out_of_stock"`, en vez de `AffiliateLink` directo — mismo criterio que ya rige `ProductDetail.tsx` desde la ronda 39.
2. `RecentlyViewed` (ya existía en ficha y `/guardados` vacío, nunca en la home — confirmado por grep) se suma a la home, con copy explícito y transparente: "Volviste — esto viste en este navegador" + "No en otros dispositivos ni cuentas: es solo lo que abriste acá."

**Verificado (build de producción real):** en la categoría gaming (53 productos, 5 sin stock), los 5 botones redondos correspondientes apuntan a `/producto/...` con aria-label "Ver alternativas para X (sin stock en esta publicación)"; los 48 restantes conservan el link directo a MercadoLibre — conteo exacto 48+5=53. En la home, sin historial el módulo nuevo no aparece (confirmado por DOM real, no por el payload RSC que sí serializa los props aunque el componente renderice null); con historial simulado, aparece con el copy correcto arriba del feed principal. `tsc --noEmit`, `eslint` y `npm run build`, todos limpios.

**Con 58 features implementadas, 58 commits locales.**

### Iteración 41 (2026-08-01) — Codex y Gemini cierran la cadena de fugas de stock, investigación de prueba social encuentra dato subutilizado

**Codex y Gemini convergieron, otra vez de forma independiente**, en cerrar las superficies estructurales que quedaban mandando directo a MercadoLibre sin mirar `priceStatus` (después de ficha, cards y alternativas en las rondas 38-40): `AboveFoldCta`/`VerdictCta` de guías, `StickyBuyBar`, `guides/ProductCard.tsx` (dos CTAs), `ComparisonTable.tsx` del modo comparar, y la fila "Ver" de "Comparar con otros modelos" en la ficha (la lista curada a mano, distinta de la automática ya filtrada desde la ronda 39). **Investigación externa (ángulo prueba social honesta):** Baymard advierte que apilar señales de confianza nuevas arriba de las 5 que ya existen (rating, reviews, ventas, mínimo histórico, descuento) puede empeorar conversión por saturación; PowerReviews y un paper de Journal of Consumer Research (Mousavi et al.) documentan que la distribución real de estrellas (a escala absoluta, no normalizada al pico) es más creíble que el promedio pelado. Encontró que `ratingBreakdown` ya existe en 60/303 productos pero nunca se renderiza — confirmado por grep, cero usos.

**Implementado (dos features):**
1. Mismo criterio ya establecido en rondas 39-40 aplicado a las 5 superficies restantes: si `priceStatus === "out_of_stock"`, el CTA queda como link interno a la ficha, nunca a MercadoLibre. Todo en componentes que renderizan, sin tocar `guides.ts`/`articleBody`/prosa/picks editoriales.
2. `ratingBreakdown` ahora se muestra en la ficha como barras de distribución a escala absoluta, colapsadas en un `<details>` dentro del bloque de rating existente (no como sección nueva, por la advertencia de saturación de Baymard). El texto mostrado conserva el string original de MercadoLibre tal cual ("70%+", "~15%"), solo se limpia para calcular el ancho de la barra.

**Verificado (build de producción real):** en la guía "silla-gamer", el product-card del Corsair T3 Rush (sin stock) muestra "Ver alternativas disponibles" con href interno; en una ficha que compara con el mouse Logitech G Pro Wireless (sin stock), la fila del comparador tiene aria-label "sin stock en esta publicación" y href interno. En la ficha del Kit Gadnic (ratingBreakdown exacto), las barras renderizan a 79.59/12.68/3.66/1.63/2.44% confirmado por DOM real; en una ficha con formato aproximado, la barra usa el número limpio (70%, 15%) pero el texto sigue mostrando "70%+"/"~15%". `tsc --noEmit`, `eslint` y `npm run build`, todos limpios. `npm run guides:check` marca 3 precios hardcodeados desactualizados en prosa, preexistentes y no relacionados a este cambio.

**Con 60 features implementadas, 60 commits locales.**

### Iteración 42 (2026-08-01) — Codex encuentra una idea genuinamente nueva, investigación del handoff confirma que el patrón actual ya es el correcto, Gemini falla

**Codex** propuso algo distinto por primera vez en varias rondas (no relacionado a stock): una navegación de atajos en la ficha hacia secciones de evidencia que ya existen más abajo (pros/contras, comparar, opiniones, specs, FAQ) pero sin forma rápida de llegar — a diferencia de las guías, que ya tienen TOC, la ficha no tenía ningún tipo de navegación in-page. **Gemini falló** de nuevo (gotcha #6). **Investigación externa (ángulo el momento exacto de salida hacia MercadoLibre):** conclusión honesta — el patrón actual del sitio (pestaña nueva + `rel="sponsored nofollow noopener"` + disclaimer de comisión inline en el CTA) es exactamente lo que recomienda la evidencia citada (FTC, Baymard, un meta-análisis de disclosure de contenido patrocinado). No encontró ninguna palanca de conversión escondida — solo un detalle chico de accesibilidad (WCAG G200/G201: avisar ANTES del click que el link abre pestaña nueva), que al verificar contra el código resultó real pero inconsistente: 3 CTAs de la ficha ya lo hacían, 7 superficies más no.

**Implementado (dos features):**
1. `DecisionNav`: fila de pills debajo del CTA principal de la ficha (nunca arriba, para no competir con la acción de compra) con atajos condicionales a las 5 secciones de evidencia, cada uno visible solo si esa sección existe para el producto puntual. `SectionCard` suma un prop `id` + `scroll-mt-20` (mismo patrón que ya usan los títulos de guía) para que el salto no quede tapado por el header sticky.
2. Alineación de aria-label en las 7 superficies que no mencionaban "(se abre en una pestaña nueva)": `products/ProductCard.tsx`, `StickyMobileCta.tsx`, `ComparisonTable.tsx`, `GuideRenderer.tsx` (2 CTAs), `guides/ProductCard.tsx` (2 CTAs), `StickyBuyBar.tsx`.

**Verificado (build de producción real + navegador):** los 5 anchors de `DecisionNav` renderizan con href correcto y sus 5 ids target existen en la misma página; en el navegador, click en "Ficha técnica" actualiza el hash, scrollea y la sección queda a 80px del top (offset correcto, no tapada por el header). El aria-label "(se abre en una pestaña nueva)" aparece ahora en las cards de "Productos similares" y en las demás superficies tocadas. `tsc --noEmit`, `eslint` y `npm run build`, todos limpios.

**Con 62 features implementadas, 62 commits locales.**

### Iteración 43 (2026-08-01) — cola realmente en cero: Codex y Gemini traen dos ideas nuevas y distintas, investigación de GEO en fichas recomienda no invertir ahí

Primera ronda en varias donde ninguna de las tres fuentes tocó el tema de stock (la cadena de las rondas 38-42 está genuinamente agotada). **Codex** propuso un filtro por marca en listados. **Gemini** propuso un atajo de teclado Ctrl/Cmd+K para el buscador. **Investigación externa (ángulo contenido citable por IA en fichas/categorías, no en guías):** encontró evidencia de que las fichas de producto individuales pierden feo contra los listicles tipo "mejores X" (13.7% de citas de IA vs 40% de citas comerciales para listicles) — recomendó explícitamente NO invertir esfuerzo pesado de GEO en las 303 fichas individuales, y como único ítem barato de verificar: que el JSON-LD Product/Offer/AggregateRating ya existente matchee EXACTAMENTE los datos visibles (por el riesgo real de manual action de Google por "spammy structured markup" declarado en julio 2026). **Verificado y ya resuelto**: el JSON-LD de `producto/[slug]/page.tsx` ya tiene ese principio como regla explícita desde antes ("los campos canónicos SIEMPRE ganan sobre structuredData manual", `aggregateRating` "solo con datos reales... nunca se inventa", precio/disponibilidad derivados en vivo de `product.price`/`priceStatus`) — nada para implementar ahí.

**Implementado (dos features):**
1. Filtro por marca en `SortableProductGrid.tsx`: `brand` sumado a `CardProduct` (no estaba, solo en `Product`). Cobertura real contada con cuidado antes de implementar (no homogénea, como advirtió Codex): 54/300 productos con marca, casi toda en belleza (36/60, Lattafa domina con 22) y algo en gaming (15/52) — el resto de las categorías queda en 0. Mismo criterio de "ocultar si no hay 2+ opciones reales" que ya usan precio y señales, así el filtro aparece solo donde tiene sentido sin casos especiales por categoría.
2. Atajo Ctrl/Cmd+K en `Header.tsx` (mismo patrón ya usado para cerrar el dropdown de categorías con Escape) que abre el buscador con foco automático (nuevo en `SearchInput.tsx`, cubre también el click normal en la lupa, que antes no enfocaba solo).
