...
- [~] **1.3 PriceAlert real (guardar email + producto)** — 🟡 Parcial 2026-07-11: el widget YA guarda el lead (POST a `/api/subscribe` con `source: "price-alert"`, loading/error/success, lock anti doble-submit, timeout, a11y). Copy honesto (radar de bajas de la tienda; alertas por producto "en camino"). Parametrizado y sumado a las **21 fichas sin stock** con captura de alta intención ("avisame cuando vuelva", `ref = productId`). **Falta** (próxima tanda): **Resend** (dep + cuenta de Juan) para mandar los mails; tabla `price_watches` por producto + cron que compara precio (Bright Data ya detecta bajas → `.cache/pending-price-drops.json`) y dispara el mail con el link de afiliado.
