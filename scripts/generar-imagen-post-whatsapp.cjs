#!/usr/bin/env node
// Genera la imagen (PNG 1080x1080, cuadrado 1:1) de un post para WhatsApp a
// partir de datos de producto ya verificados.
// Uso: node scripts/generar-imagen-post-whatsapp.cjs '<json de producto>' [salida.png]
//
// Mismo JSON que scripts/generar-imagen-post-threads.cjs (imgUrl, badge, title, brand,
// rating, sold, oldPrice, newPrice, offPct, savings, noDiscount opcional).
// Usa una plantilla propia (threads-post-template-whatsapp.html) en 1:1 —
// nunca reusar el PNG 1080x1350 del post de Threads/IG: en WhatsApp Estados
// (proporción 9:16) ese formato queda con el wordmark de arriba cortado.
//
...
const TEMPLATE_PATH = path.join(__dirname, "threads-post-template-whatsapp.html");
...
function main() {
  ...
  if (!jsonArg) {
    console.error("Uso: node scripts/generar-imagen-post-whatsapp.cjs '<json de producto>' [salida.png]");
    process.exit(1);
  }
  ...
  const tmpDir = fs.mkdtempSync(path.join(require("os").tmpdir(), "pv-whatsapp-post-"));
