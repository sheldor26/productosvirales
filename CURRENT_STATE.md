...
9. **⚠️ node_modules destruido y eliminado** (ver MISTAKES.md): un npm install vía symlink lo rompió sin arreglo. `package.json`/`package-lock.json` intactos. **Juan tiene que correr `npm install` en el proyecto antes del próximo build local.**
